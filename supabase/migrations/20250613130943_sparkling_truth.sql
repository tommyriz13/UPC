...
ALTER TABLE standings_league 
DROP CONSTRAINT IF EXISTS standings_league_edition_id_team_id_key;

ALTER TABLE standings_champions_groups 
DROP CONSTRAINT IF EXISTS standings_champions_groups_edition_id_group_name_team_id_key;

ALTER TABLE stats_league 
DROP CONSTRAINT IF EXISTS stats_league_edition_id_player_id_key;

ALTER TABLE stats_cup 
DROP CONSTRAINT IF EXISTS stats_cup_edition_id_player_id_key;

ALTER TABLE stats_champions 
DROP CONSTRAINT IF EXISTS stats_champions_edition_id_player_id_key;

-- Create new unique constraints
ALTER TABLE standings_league 
ADD CONSTRAINT standings_league_edition_id_team_id_key UNIQUE (edition_id, team_id);

ALTER TABLE standings_champions_groups 
ADD CONSTRAINT standings_champions_groups_edition_id_group_name_team_id_key UNIQUE (edition_id, group_name, team_id);

ALTER TABLE stats_league 
ADD CONSTRAINT stats_league_edition_id_player_id_key UNIQUE (edition_id, player_id);

ALTER TABLE stats_cup 
ADD CONSTRAINT stats_cup_edition_id_player_id_key UNIQUE (edition_id, player_id);

ALTER TABLE stats_champions 
ADD CONSTRAINT stats_champions_edition_id_player_id_key UNIQUE (edition_id, player_id);
...
-- First check if standings exists as a view or table and drop accordingly
DO $$ 
BEGIN
  -- Check if standings exists as a view first
  IF EXISTS (
    SELECT 1 FROM information_schema.views 
    WHERE table_name = 'standings' AND table_schema = 'public'
  ) THEN
    DROP VIEW standings CASCADE;
  -- Check if standings exists as a table
  ELSIF EXISTS (
