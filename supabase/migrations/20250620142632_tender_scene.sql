/*
  # Fix match validation for all competition types

  1. Drop all dependent constraints first
  2. Drop and recreate functions with updated logic
  3. Recreate all constraints and triggers
  4. Support validation across all match tables (league, cup, champions, legacy)
*/

-- Drop all dependent constraints first
...
-- Drop the trigger that depends on validate_result_submission
...
-- Recreate all the check constraints
